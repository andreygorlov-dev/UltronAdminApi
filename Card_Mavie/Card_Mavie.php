<?php
    ...
    class Card_Mavie extends apiBaseClass {
         
        function getMaxPosition($idPage) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetMaxPositionCard.sql"))->replace('%ID_PAGE%', $idPage)->getSql());
        ...
        function getMaxPositionContent($idCard) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetMaxPositionCardContent.sql"))->replace('%ID_CARD%', $idCard)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['POSITION'] + 1;
            }
            return 1;
        ...
        function getPosition($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetPositionCards.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['POSITION'];
            }
            return 0;
        ...
        function getPositionContent($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetPositionCardsContent.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['POSITION'];
            }
            return 0;
        ...
        function getImgSrc($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetImgSrcCard.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
        ...
        function get_card_mavie($getData, $postData) {
            ...
            if ($getData['type'] === 'preview') {
                ...
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/GetCardsContent.sql"))->replace('%ID_CARD%', $getData['id'])->getSql());
                $response = array();
                while($row = $result->fetch_assoc()) {
                    $myArray = array();
                    $myArray['ID'] = $row['ID'];
                    if ($row['TYPE'] === 'ELEM_LIST' || $row['TYPE'] === "TEXT3" || $row['TYPE'] === "TABLE" || $row['TYPE'] === "LINK" || $row['TYPE'] === "LINK2") {
        ...
        function post_card_mavie($getData, $postData) {
            if (empty($postData)) {
            ...
            if ($getData['type'] === 'preview') {
                ...
                if (!empty($postObject->position) && $pos > $postObject->position) {
                    $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/UpdatePositionCard.sql"))
                ...
                $sql = (new SqlScript("/Card_Mavie/sql/AddCard.sql"))
                ...
                switch ($postObject->type) {
                    ...
                    case 'LINK':
                        ...
                        break;    
                    case 'LINK2':
                        $value = json_encode($postObject->link2, JSON_UNESCAPED_UNICODE);
                        break;  
                    default:
                ...
                if (!empty($postObject->position) && $pos > $postObject->position) {
                    $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/UpdatePositionContentCard.sql"))
                                                                            ->replace("%FIRST_POSITION%", $postObject->position)
                                                                            ->replace("%SECOND_POSITION%", $pos)
                                                                            ->replace("%OPERATION%", "+")
                ...
                $sql = (new SqlScript("/Card_Mavie/sql/AddContentCard.sql"))
                            ->replace("%VALUE%", $value)
        ...
        function delete_card_mavie($getData, $postData) {
            ...
            if ($getData['type'] === 'preview') {
                ...
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/UpdatePositionCard.sql"))
                                                                        ...
                                                                        ->replace("%OPERATION%", "-")
                                                                        ->getSql());  
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/DeleteCard.sql"))->replace('%ID%', $getData['id'])->getSql());
            } else if ($getData['type'] === 'content') { 
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/UpdatePositionContentCard.sql"))
                                                                        ...
                                                                        ->getSql());  
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/DeleteElement.sql"))->replace('%ID%', $getData['id'])->getSql());
            } else {
                throw new InvalidArgumentException('Ошибка в праметре type');
            }
        ...
        function put_card_mavie($getData, $postData) {
            //if (!isset($getData['id']) || !isset($getData['idParent'])) {
            //    throw new InvalidArgumentException('Не указан id');
            //}
        ...
        function updateContent($getData, $postData) {
            if (empty($postData)) {
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Card_Mavie/sql/UpdateVisibilityCardContent.sql"))
            ...
            $sql = (new SqlScript("/Card_Mavie/sql/UpdateCardContent.sql"))
                                ->replace("%ID%", $getData['id']);
        ...
        function updatePreview($getData, $postData) {
            if (empty($postData)) {
                $result = $this->mySQLWorker->connectLink->query($sql = (new SqlScript("/Card_Mavie/sql/UpdateVisibilityCard.sql"))
                            ->replace("%ID%", $getData['id'])
                            ->getSql());
                return;
            }
            ...
            $sql = (new SqlScript("/Card_Mavie/sql/UpdateCard.sql"))
                                ->replace("%ID%", $getData['id']);
