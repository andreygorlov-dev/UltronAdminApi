<?php
    ...
    class Document extends apiBaseClass {

        function getMaxPosition() {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/GetMaxPositionDocuments.sql"))->getSql());
        ...
        function getImgSrc($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/GetImgSrcDocument.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['IMG_SRC'];
            }
        ...
        function getDocSrc($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/GetDocSrcDocument.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['FILE_SRC'];
            }
            throw new InvalidArgumentException('Несуществующий id страницы');;
        ...
        function getPosition($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/GetPositionDocument.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['POSITION'];
            }
            return 0;
        ...
        function get_document($getData, $postData) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/GetDocuments.sql"))->getSql());
            $myArray = array();
        ...
        function post_document($getData, $postData) {
            if (empty($postData)) {
            ...
            if (!empty($postObject->position) && $pos > $postObject->position) {
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/UpdatePositionDocument.sql"))
            ...
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/AddDocument.sql"))
        ...
        function delete_document($getData, $postData) {
            if (!isset($getData['id'])) {
            ...
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Document/sql/DeleteDocument.sql"))->replace('%ID%', $getData['id'])->getSql());
        }

        function put_document($getData, $postData) {
            if (!isset($getData['id'])) {
                throw new InvalidArgumentException('Не указан id страницы');
            }

            if (empty($postData)) {
                $result = $this->mySQLWorker->connectLink->query($sql = (new SqlScript("/Document/sql/UpdateVisibility.sql"))
            ...
            $sql = (new SqlScript("/Document/sql/UpdateDocument.sql"))
                                ->replace("%ID%", $getData['id']);
