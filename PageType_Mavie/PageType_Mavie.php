<?php
    ...
    class PageType_Mavie extends apiBaseClass {

        function getImgSrc($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/PageType_Mavie/sql/GetImgSrcPage.sql"))->replace("%ID%", $id)->getSql());
        ...
        function get_pagetype_mavie($getData, $postData) {
            if (!isset($getData['id'])) {
                throw new InvalidArgumentException('Не указан id страницы');
            }

            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/PageType_Mavie/sql/GetPageType.sql"))->replace("%ID%", $getData['id'])->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row;
            }
            return null;
        ...
        function post_pagetype_mavie($getData, $postData) {
            if (empty($postData)) {
            ...
            $sql = new SqlScript("/PageType_Mavie/sql/AddPageType.sql");
            ...
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/PageType_Mavie/sql/UpdateTypePage.sql"))
                                                                    ->replace("%ID_PAGE_TYPE%", $last_id)
                                                                    ->replace("%ID%", $getData['id'])
                                                                    ->getSql());
        }

        function delete_pagetype_mavie($getData, $postData) {
            
        }

        function put_pagetype_mavie($getData, $postData) {
        }
