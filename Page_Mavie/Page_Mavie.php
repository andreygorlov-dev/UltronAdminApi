<?php
    ...
    class Page_Mavie extends apiBaseClass {
         
        function getMaxPosition() {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/GetMaxPositionPages.sql"))->getSql());
        ...
        function getPosition($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/GetPositionPages.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['POSITION'];
            }
            return 0;
        ...
        function getImgSrc($id) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/GetImgSrcPage.sql"))->replace("%ID%", $id)->getSql());
            if ($row = $result->fetch_assoc()) {
                return $row['IMG_SRC'];
            }
        ...
        function get_page_mavie($getData, $postData) {
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/GetPages.sql"))->getSql());
            $myArray = array();
        ...
        function post_page_mavie($getData, $postData) {
            if (empty($postData)) {
            ...
            if (!empty($postObject->position) && $pos > $postObject->position) {
                $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/UpdatePositionPage.sql"))
            ...
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/AddPage.sql"))
        ...
        function delete_page_mavie($getData, $postData) {
            if (!isset($getData['id'])) {
            ...
            $result = $this->mySQLWorker->connectLink->query((new SqlScript("/Page_Mavie/sql/DeletePage.sql"))->replace('%ID%', $getData['id'])->getSql());
        }

        function put_page_mavie($getData, $postData) {
            if (!isset($getData['id'])) {
                throw new InvalidArgumentException('Не указан id страницы');
            }

            if (empty($postData)) {
                $result = $this->mySQLWorker->connectLink->query($sql = (new SqlScript("/Page_Mavie/sql/UpdateVisibility.sql"))
            ...
            $sql = (new SqlScript("/Page_Mavie/sql/UpdatePage.sql"))
                                ->replace("%ID%", $getData['id']);
